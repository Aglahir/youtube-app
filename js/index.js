// A01364026 Aglahir Jimenez Florez
...
function displayResults(jsonIn, term)
{
    ...
    let pageButtons = '<div class="pageButtons">';
  
    if(jsonIn.prevPageToken != null){
      pageButtons += `
          <button class="pageButton" onclick="fetchVideos('${term}', '${jsonIn.prevPageToken}')">Prev</button>
      `;
    }
    
    pageButtons += `
        <button class="pageButton" onclick="fetchVideos('${term}', '${jsonIn.nextPageToken}')">Next</button>
      </div>
    `;
  
  results.innerHTML += pageButtons;
}
